...
%In this example, Efinder will find the eigen energies that satisfy the 
%Schroedinger equation for the following potential, which is encountered when 
%dealing with trapping a fluxon in an Long Josephson Junction among other 
%places: 

%               potential=(-.2)*((sech(tt)).^2)

%Then it will calculate the exact value of En and compare this value with 
%the calculated value to illustrate the fact that Efinder's accuracy can 
%be adjusted to whatever accuracy is required by changing the variable
...
m =  8;            %enter the 'mass' used in the Schroedinger equation

precision=6;      %will give data correct up to the sixth decimal place in
                  %this case. Change it accordingly.
...
steps=     200      ;  %enter how many increments the above range should be 
...
%__________________________________________________________________________
%This part calculates the exact value for the eigen energies in this
%example, as the resulting differential equation yields an exact formulaic
%expression for En. This value is then compared with the value calculated
%by the program Efinder to illustrate the fact that Efinder's accuracy can 
%be adjusted to whatever accuracy is required by changing the variable
%'precision' appropriately.
...
ep=.1;
nn=0;
while ((1+(16*ep*8))^(1/2))>(1+2*nn)
    nn;
    e=-(1/64)*(((1+(16*ep*8))^(1/2))-(1+2*nn))^2;
    actual=[actual; e];
    nn=nn+1; 
end
...
fprintf(1,'   %11.10f  \n',datal);
fprintf(1,'   \n');
fprintf(1,'This next set of data are the exact eigen energies for this \n');
fprintf(1,'potential to be compared with the data immediately above:\n');
fprintf(1,'   \n');
fprintf(1,'   %11.10f  \n',actual);
fprintf(1,'   \n');
fprintf(1,'This next set of data are the difference between the calculated \n');
fprintf(1,'and the actual\n');
fprintf(1,'   \n');
fprintf(1,'   %11.10f  \n',a);
fprintf(1,'   \n');
...
%The function schrod stores the Schroedinger equation. Note: hbar has been 
%scaled to 1. Enter the potential below like: 
