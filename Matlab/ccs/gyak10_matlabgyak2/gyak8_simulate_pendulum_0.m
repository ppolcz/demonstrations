...
if nargin == 3 && isnumeric(u)
    pause(0.1)
    subplot(2,2,[3 4]), hold off
    plot_pendulum_arrow_vec(1);
    pause(0.5)
    
    tic;
    for kk = 1:length(t_rs)
        plot_pendulum_arrow_vec(kk)
        pause(Ts - toc)
        tic
    end
else
    ...
    function plot_pendulum(kk)
        ...
        text(6,-1.5,sprintf('t = %g [s]', round(t_rs(kk))));
        hold off;
    end

    function plot_pendulum_arrow_vec(kk)
        plot([r_init(kk)-cl/2, r_init(kk)-cl/2, r_init(kk)+cl/2, r_init(kk)+cl/2, r_init(kk)-cl/2], [-cw/2, cw/2, cw/2, -cw/2, -cw/2])
        hold on;
        plot([r_init(kk); x1(kk)],[0; y1(kk)],'b-'),
        plot(x1(kk), y1(kk), 'o')
        quiver(r_init(kk), -0.2, u_rs(kk), 0, 'Color', [161, 15, 7]/255, 'LineWidth', 2)
        axis([-13 13 -2 2]), grid on;
        text(6,-1.5,sprintf('t = %g [s]', round(t_rs(kk))));
        hold off;
    end
