function [t,p,dp] = pcz_pdp_lim_smooth_normalized(T, dt, varargin)
%% pcz_pdp_lim_smooth_v2
%  
%  File: pcz_pdp_lim_smooth_normalized.m
...
for proba = 1:opts.maxnrit
    ...
    if 0.8 < ROSSZASAG && ROSSZASAG < 1.5
        break
    end
...
if opts.plot
    figure(1)
    ax(1) = subplot(211); plot(t,p), grid on
    title('p')
    xlim(t([1,end]))
    ylim(p_lim)

    grid on
    ax(2) = subplot(212); plot(t,dp), grid on
    title('dp') 
    xlim(t([1,end]))
    ylim(dp_lim)
...
[t,p,dp] = pcz_pdp_lim_smooth_normalized(T,dt,'eltoldp',false,'plot',true);
