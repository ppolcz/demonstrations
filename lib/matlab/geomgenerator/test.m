...
for example = 1:18
    if (example == 1) % rectangle
        L = 1;
        H = 0.1;
        xy = [0 0; L 0; L H; 0 H];
        pdeGeom = geomDataFromPolygon(xy);
    elseif (example == 2) % a convex polygon
        xy = [0 0; 1 0; 0.9999 0.1; 0.5 0.65; 0.1 0.5];
        pdeGeom = geomDataFromPolygon(xy);
    elseif (example == 3) % L Bracket with rounded corners
        L = 1;
        H1 = 0.1;
        H2 = 0.1;
        R = 0.1;
    ...
    elseif (example == 4) % rectangle with square hole
        L = 1;
        H = 0.4;
        a = H/4;
    ...
    elseif (example == 5) % rectangle with  semidisc cutout
        L = 1;
        h = 0.2;
        r = h/3;
        yc = 0;
    ...
    elseif (example == 6) % diverging/converging nozzle
        L = 1;
        h = 0.2;
        g = 0.1;
        if (g <= h)
    ...
    elseif (example == 8) % U Channel
        ...
        pdeGeom = geomDataFromPolygon(xy);
    elseif (example == 9) % Bow-tie
        L = 1;
        t = 0.5;
        g = 0.1;
        xy = [-L/2 -t/2; 0 -g/2; L/2 -t/2; L/2 t/2; 0 g/2; -L/2 t/2];
        pdeGeom = geomDataFromPolygon(xy);
    elseif (example == 10) % plate with circular hole
        L = 1;
        H = 0.1;
        R = H/100;
        xyRect = [-L/2 -H/2; L/2 -H/2; L/2 H/2; -L/2 H/2];
    ...
    elseif (example == 11) % square plate with many square holes
        L = 1;
        nHolesX = 2;
        nHolesY = 2;
    ...
    elseif (example == 12) % square plate with many circular holes
        L = 1;
        nHoles = 1;
    ...
    elseif (example == 13) % traffic circle
        L = 1; % half length
        h = 0.1; % thickness
        R1 = 0.4; % outer circle
        R2 = 0.2; % inner circle
    ...
    elseif (example == 14) % stepped
        [pdeGeom] = constructStep(1,1,0.2,0.1,0.02);
    elseif (example == 15) % high order bisector
        L = 1.0;
        H = 2.0;
        R = 3;
        Yc = sqrt(R*R - L*L) - H;
